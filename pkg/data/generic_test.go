...
func (tc testCase) testGenerateSyncPayload(t *testing.T) {
	data := make([]unstructured.Unstructured, 0, len(tc.objs))
	for _, obj := range tc.objs {
		data = append(data, unstructured.Unstructured{Object: mustJSONRoundTrip(t, obj)})
	}
...
func (tc *testCase) play(t *testing.T, scheme *runtime.Scheme, objs []runtime.Object, play script) *mockData {
	...
	mock := &mockData{}
	sync := NewFromInterface(client, mock.Prefix(tc.prefix), tc.resourceType)
