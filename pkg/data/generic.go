...
type GenericSync struct {
	...
	opa    opa_client.Data
	ns     types.ConstrainedResourceType
}
...
func New(client dynamic.Interface, opa opa_client.Data, ns types.ConstrainedResourceType) *GenericSync {
	return &GenericSync{
		client: client,
		ns:     ns,
...
func (s *GenericSync) sync(store cache.Store, queue workqueue.Interface) error {
	queue.Add(initPath) // This key bootstraps the cache
	init := false
	for !queue.ShuttingDown() {
		...
		err := s.processNext(store, key, &init)
		queue.Done(key)
		if err != nil {
			return err
		}
...
func (s *GenericSync) processNext(store cache.Store, key interface{}, init *bool) error {
	path, err := s.objPath(key)
	if err != nil {
		return err
	}
...
func (s *GenericSync) generateSyncPayload(objs []interface{}) (map[string]interface{}, error) {
...
func (s *GenericSync) objPath(key interface{}) (string, error) {
